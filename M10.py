...
# Enhanced function to extract text from PDF
...
# Function to create temporary files for LangChain processing
...
if uploaded_files and openai_api_key:
    ...
    if current_filenames != st.session_state.uploaded_file_names:
        ...
        with st.spinner("Processing PDF files..."):
            # Create a list to store all documents for vector store creation
            all_docs = []
            ...
            for pdf_file in uploaded_files:
                ...
                if extracted_text:
                    ...
                    # Save file temporarily for LangChain processing
                    ...
                    if temp_file_path:
                        try:
                            ...
                            # Add file information to each page
                            for page in pages:
                            ...
                            st.sidebar.warning(f"Error in LangChain processing: {e}")
...
# Enhanced RAG function using LangChain when available, fallback to direct search
def generate_rag_answer(question, documents, course_content=None):
    try:
        if not openai_api_key:
            return "API key is required to generate answers."
        
        if not documents and not st.session_state.vector_store:
            return "Document text is not available. Please process documents first."
        
        # Try using LangChain's retrieval if vector store exists
        if st.session_state.vector_store:
            try:
                ...
                if course_content:
                    course_context = f"""
                    Consider this course information in your answer:
                    Course Title: {course_content.get('course_title', '')}
                    Course Description: {course_content.get('course_description', '')[:300]}
                    ...
                    enhanced_question = f"{course_context}\n\nQuestion: {question}"
                else:
                ...
                st.warning(f"LangChain retrieval error: {e}. Falling back to direct method.")
                # Fall back to the direct method if LangChain fails
...
if st.sidebar.button("Submit Question"):
    if new_query:
        ...
        answer = ""
        if st.session_state.extracted_texts or st.session_state.vector_store:
