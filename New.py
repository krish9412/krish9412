...
def init_session_state():
    ...
    # Chroma specific - collection name and persistence directory
    if 'chroma_collection_name' not in st.session_state:
        st.session_state.chroma_collection_name = f"collection_{st.session_state.user_id}"
    if 'chroma_persist_directory' not in st.session_state:
        st.session_state.chroma_persist_directory = f"./chroma_db_{st.session_state.user_id}"
...
def create_knowledge_base():
    """Create a searchable knowledge base from processed documents using Chroma"""
    ...
    try:
        ...
        for doc_idx, doc in enumerate(st.session_state.learning_materials):
            document_chunks = text_splitter.split_text(doc["content"])
            # Create metadata for each chunk
            metadatas = [{"source": doc["filename"], "doc_id": doc_idx} for _ in range(len(document_chunks))]
        ...
        st.success("✅ Knowledge base successfully created with Chroma!")
        return True
...
def ask_knowledge_base(question):
    """Generate answer to question using RAG with Chroma"""
    try:
        if not st.session_state.knowledge_base:
